...
//        Criação do método loadImage, e da hash images.
//        Em todas as chamadas para a criação de uma nova imagem, foi substituida
//        pela chamada de loadImage que antes de criar a imagem verifica se ela já não existe na hash.

        private BufferedImage loadImage(String strImage){

            if(images.get(strImage)!= null){
                return images.get(strImage);
            }else{
                try{
                    BufferedImage img = ImageIO.read(new File(strImage));
                    images.put(strImage,img);
                    return img;
                } catch (IOException e) {
                    e.printStackTrace();
                    return null;
            ...
            if (Table.get().getHighlightLegalMoves()) {
                for (final Move move : pieceLegalMoves(board)) {
                    if (move.getDestinationCoordinate() == this.tileId) {
                        add(new JLabel(new ImageIcon( loadImage("art/misc/green_dot.png"))));
                    }
                }
            }
        }
        ...
        private void assignTilePieceIcon(final Board board) {
            ...
            if(board.getTile(this.tileId).isTileOccupied()) {
                ...
                final BufferedImage image = loadImage(pieceIconPath +
